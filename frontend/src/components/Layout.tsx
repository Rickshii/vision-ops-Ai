...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ backgroundColor: 'hsl(var(--background))' }}>
      ...
      {/* SIDEBAR */}
      <aside
        className={`hidden md:flex flex-col relative z-10 transition-all duration-300 ease-in-out glassmorphism border-r border-white/5 ${
      ...
      >
        ...
        {/* Logo */}
        <div className={`flex items-center h-16 border-b border-white/5 px-4 ${isCollapsed ? 'justify-center' : 'gap-3 px-5'}`}>
          ...
          {!isCollapsed && (
            <div className="flex flex-col overflow-hidden">
              <span className="font-black text-sm tracking-tight leading-none text-white">
            ...
            return (
              <Link
                ...
                className={`flex items-center rounded-xl text-sm font-medium transition-all duration-200 group relative overflow-hidden ${
                  ...
                  isActive
                    ? 'bg-gradient-to-r from-[#A855F7]/20 to-[#EC4899]/10 text-white border border-[#A855F7]/25 shadow-[0_0_15px_rgba(168,85,247,0.1)]'
        ...
        <button
          ...
          className="absolute -right-3 top-20 h-6 w-6 rounded-full glassmorphism border border-white/10 flex items-center justify-center text-muted-foreground hover:text-slate-100 transition-all duration-200 hover:border-[#A855F7]/40 shadow-lg"
        >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative z-10">
        ...
        {/* TOP NAV */}
        <header className="h-16 border-b border-white/5 glassmorphism flex items-center justify-between px-4 md:px-6 flex-shrink-0">
          <div className="flex items-center gap-3">
            {/* Mobile hamburger */}
            <button
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="md:hidden p-2 text-muted-foreground hover:text-slate-100 hover:bg-white/5 rounded-xl transition-all"
            >
              <Menu className="h-5 w-5" />
            </button>
            {/* Page Title */}
            <div className="flex flex-col">
              <h1 className="text-base font-bold text-slate-100 leading-none">{currentPageName}</h1>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2 text-muted-foreground hover:text-slate-100 hover:bg-white/5 rounded-xl transition-all border border-transparent hover:border-white/10"
            ...
            <div className="relative">
              <button
                ...
                className="p-2 text-muted-foreground hover:text-slate-100 hover:bg-white/5 rounded-xl relative transition-all border border-transparent hover:border-white/10"
              >
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-80 rounded-2xl glass-panel-purple z-50 shadow-[0_25px_50px_rgba(0,0,0,0.4)] animate-fade-in-up border border-[#A855F7]/20">
                    <div className="p-4 border-b border-white/5 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-slate-100">Live AI Detections</span>
                        <Badge variant="danger" className="text-[9px] px-1.5 py-0">{notifications.length}</Badge>
                      </div>
                      <button onClick={() => setIsNotifOpen(false)} className="p-1 hover:bg-white/10 rounded-lg text-muted-foreground transition-colors">
                    ...
                    <div className="max-h-64 overflow-y-auto divide-y divide-white/5">
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((n) => (
                          <div key={n.id} className="p-3 hover:bg-white/[0.03] transition-colors">
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-[11px] font-bold text-slate-200">{n.cameraName}</span>
          ...
          <div className="relative flex flex-col w-72 glass-panel-purple p-4 z-10 h-full overflow-y-auto">
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <span className="font-black text-white text-sm">Vision<span className="neon-gradient-text">Ops</span> AI</span>
              </div>
              <button
                onClick={() => setIsMobileMenuOpen(false)}
                className="p-1.5 text-muted-foreground hover:text-slate-100 rounded-lg hover:bg-white/10"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${
                      isActive
                        ? 'bg-gradient-to-r from-[#A855F7]/20 to-[#EC4899]/10 text-white border border-[#A855F7]/25'
                        : 'text-muted-foreground hover:text-slate-100 hover:bg-white/5'
                    }`}
                  >
            ...
            <div className="border-t border-white/5 pt-4 mt-4 space-y-2">
              <button
                onClick={() => setIsDark(d => !d)}
                className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:text-slate-100 hover:bg-white/5 transition-all w-full"
              >
                {isDark ? <Sun className="h-4 w-4 text-amber-400" /> : <Moon className="h-4 w-4 text-[#C084FC]" />}
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-sm font-bold text-slate-100">AI Alert: {toast.type}</h4>
                  <p className="text-xs text-slate-300 mt-0.5">{toast.cameraName}</p>
                  <p className="text-[10px] text-slate-400 mt-1 font-mono">{new Date(toast.timestamp).toLocaleTimeString()}</p>
                </div>
              </div>
            ...
            {toast.snapshotUrl && (
              <div className="mt-3 rounded-xl overflow-hidden aspect-video border border-white/10 bg-black/40">
